...
if __name__ == "__main__":
    name = str(sys.argv[1])
    num_classes = 72
    training_data =  torch.tensor(pd.read_csv("data/%s_input_w01.csv"%name, header=None).values, dtype=torch.float)
    ...
    df = pd.DataFrame(sol)
    df.to_csv("data/%s_w01_baseline_JS.csv"%name, header=False, index=False)
