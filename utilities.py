...
def get_data_batches(signatures, batch_size, n_samples=500):
    ...
    for i in range(batch_size):
        labels = np.random.randint(0, len(signatures)-1, size = 2)
        w = random.uniform(0, 1)
        label_weights = np.zeros(72)
        label_weights[labels[0]] = w
        label_weights[labels[1]] = 1-w
        label_weights = torch.from_numpy(label_weights) 

        signature = signatures[labels[0]]*w + signatures[labels[1]]*(1-w)
