...
def read_data(device,
    ...
    train_input = csv_to_tensor(path + "/train_%s_input.csv" % source, device)[train_range[0]:train_range[1], :]
    train_baseline = csv_to_tensor(path + "/train_%s_baseline.csv" % source, device)[train_range[0]:train_range[1], :] if include_baseline else None
    train_label = csv_to_tensor(path + "/train_%s_label.csv" % source, device)[train_range[0]:train_range[1], :] if include_labels else None

    train_data = DataPartitions(inputs=train_input,
                                prev_guess=train_baseline,
                                labels=train_label)

    val_input = csv_to_tensor(path + "/val_%s_input.csv" % source, device)[val_range[0]:val_range[1], :]
    val_baseline = csv_to_tensor(path + "/val_%s_baseline.csv" % source, device)[val_range[0]:val_range[1], :] if include_baseline else None
    val_label = csv_to_tensor(path + "/val_%s_label.csv" % source, device)[val_range[0]:val_range[1], :] if include_labels else None
